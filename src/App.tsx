...
function blankWeekPlan(weekStart: Date): WeekPlan {
  ...
  const days: DayPlan[] = Array.from({ length: 7 }).map((_, i) => {
    ...
    (MEALS as readonly string[]).forEach((m) => (meals[m] = { items: [] }));
...
export default function App() {
  ...
  const vegSummary = useMemo(() => {
    ...
    plan.days.forEach((day) => {
      (MEALS as readonly string[]).forEach((m) => {
  ...
  function addDish() {
    ...
    const dish: Dish = {
      ...
      ingredients: (newDish.ingredients as any as DishIngredient[]) || [],
      tags: newDish.tags || [],
      note: newDish.note || "",
    };
  ...
  function generateMarkdown() {
    ...
    lines.push("");
    (DAYS as readonly string[]).forEach((d, i) => {
              ...
              <table className="min-w-full border-separate border-spacing-0">
                <thead>
                  <tr>
                    ...
                    {(MEALS as readonly string[]).map((m) => (
                ...
                <tbody>
                  {(DAYS as readonly string[]).map((d, i) => (
        ...
        <div className="fixed inset-0 bg-black/40 flex items-end md:items-center justify-center z-50" onClick={() => setPickerOpen(null)}>
          <div className="bg-white w-full md:max-w-3xl rounded-t-2xl md:rounded-2xl p-4 max-h-[85vh] overflow-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-3">
              <div className="font-semibold">料理を追加（{(DAYS as readonly string[])[pickerOpen.dayIdx]} / {pickerOpen.meal}）</div>
